 ...
 */
export const totalPrice = (gifts : {name: string; price: number}[]) => {
    let total = 0
    gifts.forEach(e => {
        total += e.price;
    })
 ...
 */
export const mostExpensive = (gifts : {name: string; price: number}[]) => {
 ...
 * @returns {string} gift's name
 */
export const globalMostExpensive = (kids : {name: string; gifts: {name: string; price: number}[]}[]) => {
    let array : {name: string; price: number}[] = [];
    kids.forEach(kid => {
        kid.gifts.forEach(gift => {
            array.push(gift)
        })
    })
 ...
 */
export const preferedKid = (kids : {name: string; gifts: {name: string; price: number}[]}[]) => {
    ...
    array = array.sort((a,b) => b.price - a.price);
    let bestGift = array[0].name;
    let preferedKid = ""; 
    kids.forEach(kid => {
        kid.gifts.forEach(gift => {
            if (gift.name === bestGift) preferedKid = kid.name
        })
    })
    return preferedKid
};
 ...
 * // gifts is now []
 */
export const distributeGifts = (
  gifts: { name: string; price: number }[],
  kids: { name: string; behaviorScore: number; gifts: { name: string; price: number }[] }[]
) => {

if (gifts.length === 0) return kids
if (kids.length === 0) return kids

  const kidsWithTotal = kids.map(kid => ({
    ...kid,
    totalGifts: kid.gifts.reduce((sum, g) => sum + g.price, 0)
  }));

  gifts.sort((a, b) => b.price - a.price);

  while (gifts.length > 0) {
    kidsWithTotal.sort((a, b) => {
      if (a.totalGifts !== b.totalGifts) {
        return a.totalGifts - b.totalGifts;
      }
      return b.behaviorScore - a.behaviorScore;
    });

    const kid = kidsWithTotal[0];
    const gift = gifts.shift()!;
    kid.gifts.push({ name: gift.name, price: gift.price });
    kid.totalGifts += gift.price;
  }

  return kidsWithTotal;
};

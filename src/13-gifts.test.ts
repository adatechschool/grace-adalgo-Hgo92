...
import {totalPrice, preferedKid, distributeGifts, mostExpensive, globalMostExpensive} from "./13-gifts.js";

suite("totalPrice", () => {
    test("do the sums of two prices", () => {
        expect(totalPrice( [{name: 'iPhone', price: 800}, {name: 'skateboard', price: 70}])).toBe(870);
    });

    test("do the sums of three prices", () => {
        expect(totalPrice( [{name: 'iPhone', price: 800}, {name: 'skateboard', price: 70}, {name : 'voiture', price : 9999}])).toBe(10869);
    })
});

suite("mostExpensive", () => {
    test("Find most expensive", () => {
        expect(mostExpensive( [{name: 'iPhone', price: 800}, {name: 'skateboard', price: 70}])).toBe("iPhone");
    });
    test("Find most expensive", () => {
        expect(mostExpensive( [{name: 'iPhone', price: 800}, {name: 'skateboard', price: 70}, {name : 'voiture', price : 9999}])).toBe("voiture");
    })
});

suite("globalMostExpensive", () => {
    test("Find the most expensive", () => {
        expect(globalMostExpensive([{name: 'Issa', gifts: [{name: 'iPhone', price: 800}, {name: 'skateboard', price: 70}]},{name: 'Noor', gifts: [{name: 'apple', price: 1} ]}])).toBe("iPhone");
    });
});
...
suite("preferedKid", () => {
    ...
    test("properly use the sum of all gifts to choose", () => {
        ...
        expect(preferedKid(kids)).toBe("Nina");
    });
...
suite("distributeGifts", () => {
    ...
    test("mutates the gifts array by emptying it", () => {
        const gifts: {name : string, price : number} [] = [
            {name: "Car", price: 50},
            {name: "Ball", price: 10},
        ];

        const kids : {name : string, behaviorScore : number, gifts : {name : string, price : number}[]}[] = [
    ...
    test("kids can start with pre-filled gifts and accumulate correctly", () => {
        const gifts : {name : string, price : number}[] = [{name: "Toy", price: 30}];
    ...
    test("handles empty gifts gracefully", () => {
        const gifts : [] = [];
        const kids  : {name : string, behaviorScore : number, gifts : {name : string, price : number}[]}[] = [{name: "A", behaviorScore: 1, gifts: []}];
    ...
    test("deterministic output: same input → same result", () => {
        ...
        const kids1 : {name : string, behaviorScore : number, gifts : {name : string, price : number}[]}[] = [
            {name: "X", behaviorScore: 1, gifts: []},
            {name: "Y", behaviorScore: 0.5, gifts: []},
        ];
        const kids2 : {name : string, behaviorScore : number, gifts : {name : string, price : number}[]}[] = clone(kids1);
